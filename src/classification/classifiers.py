...
from src.classification.embeddings import get_marker_embeddings, get_marker_embeddingsDoubleMarkers
...
class EmbeddingClassifierDoubleMarkers(nn.Module):
    def __init__(self, emb_model, lbl_model, classes):
        super().__init__()
        # internal models
        self._emb = emb_model
        self._emb_pooling = get_marker_embeddingsDoubleMarkers
        self._lbl = lbl_model
        # internal variables
        self._classes = classes
        # move model to GPU if available
    ...
    def forward(self, sentences, entities_1, entities_2, entities_1end, entities_2end):
        # embed sentences (batch_size, seq_length) -> (batch_size, max_length, emb_dim)
        emb_tokens, att_tokens, encodings = self._emb(sentences)

        # prepare sentence embedding tensor (batch_size, emb_dim)
        emb_sentences = torch.zeros((emb_tokens.shape[0], emb_tokens.shape[2]*4), device=emb_tokens.device)
        # iterate over sentences and pool relevant tokens
        for sidx in range(emb_tokens.shape[0]):
            emb_sentences[sidx, :] = self._emb_pooling(emb_tokens[sidx, :torch.sum(att_tokens[sidx]), :], encodings[sidx], entities_1[sidx], entities_2[sidx], entities_1end[sidx], entities_2end[sidx])
...
class MultiLayerPerceptronClassifierDouble(EmbeddingClassifierDoubleMarkers):

    def __init__(self, emb_model, classes):

        lbl_model = nn.Sequential(
            nn.Linear(emb_model.emb_dim*4, len(classes))
        )
